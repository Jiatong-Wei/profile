...
const MUSIC_FILES = [withSiteBase('/music/close-to-you.mp3')]
...
export default function MusicCard() {
	...
	const position = useMemo(() => {
		...
		return {
			x: styles.offsetX !== null ? center.x + styles.offsetX : snapToHomeGrid(center.x + CARD_SPACING + hiCardStyles.width / 2 - styles.offset),
			y:
				styles.offsetY !== null
					? center.y + styles.offsetY
					: snapToHomeGrid(center.y - clockCardStyles.offset + CARD_SPACING + calendarCardStyles.height + CARD_SPACING)
		}
		...
		<HomeDraggableLayer cardKey='musicCard' x={x} y={y} width={styles.width} height={styles.height}>
			<Card
				...
				{siteContent.enableChristmas && (
					<>
						<img
							src={withSiteBase('/images/christmas/snow-10.webp')}
							alt='Christmas decoration'
							className='pointer-events-none absolute'
							style={{ width: 120, left: -8, top: -12, opacity: 0.8 }}
						/>
						<img
							src={withSiteBase('/images/christmas/snow-11.webp')}
							alt='Christmas decoration'
							className='pointer-events-none absolute'
				...
				<div className='flex-1'>
					...
					<div className='mt-1 h-2 overflow-hidden rounded-full bg-white/60'>
						<div
							className='bg-linear h-full origin-left rounded-full transition-transform duration-300'
				...
				<motion.button
					...
					className='flex h-10 w-10 items-center justify-center rounded-full border border-white/80 bg-white/85 shadow-sm'>
