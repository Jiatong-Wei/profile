...
import type { ImageItem } from './image-upload-dialog'
import { withSiteBase } from '@/lib/site-path'
import { APPLE_EASE_OUT } from '@/lib/motion'

const ImageUploadDialog = lazy(() => import('./image-upload-dialog'))
...
export function getProjectKey(project: Project): string {
	return project.url?.trim() || project.name
}
...
export function ProjectCard({ project, index = 0, isEditMode = false, onUpdate, onDelete }: ProjectCardProps) {
	...
	return (
		<motion.div
			...
			transition={{ duration: 0.22, delay: Math.min(index, 6) * 0.04, ease: APPLE_EASE_OUT }}
			className='card glass-standard relative flex flex-col gap-4 rounded-[var(--radius-card)]'>
			{isEditMode && (
			...
			<div className='flex items-start gap-4'>
				<div className='project-logo-frame group relative'>
					<img
						src={localProject.image.startsWith('/') ? withSiteBase(localProject.image) : localProject.image}
						alt={localProject.name}
						loading='lazy'
						decoding='async'
						width={64}
						height={64}
						className={cn('h-full w-full rounded-xl object-contain p-2', canEdit && 'cursor-pointer')}
				...
				<div className='min-w-0 flex-1'>
					<h3
						...
						className={cn('text-lg leading-snug font-semibold', canEdit && 'cursor-text focus:outline-none')}>
						{localProject.name}
					</h3>
					<div className='mt-2 flex flex-wrap items-center gap-x-2 gap-y-1.5'>
						{canEdit ? (
							...
							<span className='text-secondary text-xs font-medium'>{localProject.year}</span>
						)}
						<span className='bg-secondary/35 h-1 w-1 rounded-full' aria-hidden='true' />
						{canEdit ? (
							<input
								type='text'
							...
							localProject.tags.map(tag => (
								<span key={tag} className='text-secondary bg-card rounded-lg px-1.5 py-0.5 text-xs'>
									{tag}
								</span>
							))
						)}
