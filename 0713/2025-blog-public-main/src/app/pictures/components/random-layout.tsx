...
import { APPLE_EASE_OUT, PRESSABLE_HOVER, PRESSABLE_TAP, SPRING_LAYOUT } from '@/lib/motion'
		...
		<>
			{isZoomed && (
				<motion.div
					...
					className='dialog-modal-overlay fixed inset-0 z-50 flex items-center justify-center p-4'
				/>
			)}
			<motion.div
				drag={!isZoomed}
				...
				whileHover={!isEditMode && !isZoomed ? PRESSABLE_HOVER : undefined}
				whileTap={!isEditMode && !isZoomed ? PRESSABLE_TAP : undefined}
				className={cn('group pointer-events-auto absolute origin-center -translate-1/2 cursor-pointer shadow-xl')}>
				<motion.img
					src={url}
					onLoad={event => {
				...
				{isEditMode && !isZoomed && (
					<motion.button
						...
						aria-label='删除图片'
						className='absolute -top-2 -right-2 rounded-full bg-red-500 p-1.5 opacity-0 shadow-lg transition-[opacity,transform,background-color] group-hover:opacity-100 hover:scale-[1.02] hover:bg-red-600'
						style={{ zIndex: 1 }}>
			...
			{isZoomed && description && (
				<motion.div
					...
					dragMomentum={false}
					className='glass-panel glass-primary fixed min-h-[150px] w-[220px] cursor-pointer rounded-[18px] p-6 shadow'
					style={{
