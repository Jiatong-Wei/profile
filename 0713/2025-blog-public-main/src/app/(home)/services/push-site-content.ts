...
import { resizeImageToBase64 } from '@/lib/image-optimize-client'
...
export async function pushSiteContent(
	...
	if (faviconItem?.type === 'file') {
		...
		for (const { path, size } of [
			...
			const contentBase64 = await resizeImageToBase64(faviconItem.file, { width: size, height: size, type: 'image/png', fit: 'cover' })
	...
	if (avatarItem?.type === 'file') {
		...
		const [webpBase64, pngBase64] = await Promise.all([
			resizeImageToBase64(avatarItem.file, { width: 320, type: 'image/webp', quality: 0.84 }),
			resizeImageToBase64(avatarItem.file, { width: 320, type: 'image/png' })
		])
		const [webpBlob, pngBlob] = await Promise.all([
			createBlob(token, GITHUB_CONFIG.OWNER, GITHUB_CONFIG.REPO, webpBase64, 'base64'),
			createBlob(token, GITHUB_CONFIG.OWNER, GITHUB_CONFIG.REPO, pngBase64, 'base64')
		])
		treeItems.push(
			{ path: 'public/images/avatar.webp', mode: '100644', type: 'blob', sha: webpBlob.sha },
			{ path: 'public/images/avatar.png', mode: '100644', type: 'blob', sha: pngBlob.sha }
		)
	}
