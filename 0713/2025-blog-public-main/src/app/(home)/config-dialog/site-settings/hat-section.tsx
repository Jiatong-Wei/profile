					...
					return (
						<div key={index} className='relative'>
							<button
								...
								className={`block w-full overflow-hidden rounded-xl border bg-white/60 transition-[background-color,border-color,box-shadow,transform] duration-[160ms] ${
									isActive ? 'ring-brand shadow-md ring-2' : 'hover:border-brand/60'
								}`}>
								<img src={withSiteBase(`/images/hats/${index}.webp`)} alt={`hat ${index}`} className='h-20 w-full object-contain' />
							</button>
							{isActive && (
