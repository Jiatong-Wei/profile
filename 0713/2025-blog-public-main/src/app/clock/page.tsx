...
import { APPLE_EASE_OUT, PRESSABLE_HOVER, PRESSABLE_TAP, SELECTION_SPRING, pageReveal } from '@/lib/motion'
...
export default function ClockPage() {
	...
	return (
		<>
			<div className='public-page flex flex-col items-center px-6 pt-28 max-sm:px-4 max-sm:pt-7'>
				<PageHeader title='时钟' meta={mode === 'stopwatch' ? '秒表' : '计时器'} className='max-w-[600px]' />
				<motion.div variants={pageReveal} initial='hidden' animate='visible' className='w-full max-w-[600px] space-y-6'>
					{/* Mode Selector */}
					<div className='segmented-control glass-panel glass-quiet relative flex gap-1 p-1'>
						<motion.button
							onClick={() => {
								setMode('stopwatch')
								setIsRunning(false)
							...
							className={cn('segmented-option flex-1 py-2.5 text-sm', mode === 'stopwatch' && 'is-selected')}>
							{mode === 'stopwatch' && <motion.span layoutId='clock-mode-selection' className='segmented-selection' transition={SELECTION_SPRING} />}
							<span>秒表</span>
						</motion.button>
						<motion.button
							onClick={() => {
							...
							className={cn('segmented-option flex-1 py-2.5 text-sm', mode === 'timer' && 'is-selected')}>
							{mode === 'timer' && <motion.span layoutId='clock-mode-selection' className='segmented-selection' transition={SELECTION_SPRING} />}
							<span>计时器</span>
						</motion.button>
					</div>

					<motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className='card glass-primary relative rounded-[18px] p-4'>
						<div
							role='timer'
							aria-live='off'
							aria-label={`当前计时 ${formatTime(displayTime)}`}
							className='flex items-center justify-center rounded-[14px] border border-white/70 bg-white/36 p-8 shadow-inner'>
					...
					{mode === 'timer' && !isRunning && timerTime === 0 && (
						<motion.div
							...
							className='card glass-standard relative space-y-4 rounded-[18px]'>
					...
					<div className='flex items-center justify-center gap-4'>
						{mode === 'stopwatch' && (
							<motion.button
								...
								onClick={handleLap}
								disabled={!isRunning}
								className='flex h-16 w-16 items-center justify-center rounded-full border bg-white/60 text-sm font-medium backdrop-blur-sm transition-colors hover:bg-white/80 disabled:cursor-not-allowed disabled:opacity-50'>
								计次
							</motion.button>
						)}
						<motion.button
							...
							disabled={!canStart}
							aria-label={`${isRunning ? '暂停' : '开始'}${mode === 'stopwatch' ? '秒表' : '计时器'}`}
							title={isRunning ? '暂停' : '开始'}
							className={`flex h-20 w-20 items-center justify-center rounded-full shadow-lg transition-[background-color,box-shadow] disabled:cursor-not-allowed disabled:opacity-50 ${
								isRunning ? 'bg-brand-secondary text-primary hover:bg-brand-secondary/80' : 'bg-brand hover:bg-brand-ink text-white'
							}`}>
							<AnimatePresence mode='wait' initial={false}>
								<motion.span
									...
									transition={{ duration: 0.14, ease: APPLE_EASE_OUT }}>
						...
						<motion.button
							...
							aria-label={`重置${mode === 'stopwatch' ? '秒表' : '计时器'}`}
							title='重置'
							className='flex h-16 w-16 items-center justify-center rounded-full border bg-white/60 backdrop-blur-sm transition-colors hover:bg-white/80 disabled:cursor-not-allowed disabled:opacity-50'>
						...
						<div className='grid grid-cols-3 gap-3'>
							{laps.map((lap, index) => (
								<motion.div
									...
									animate={{ opacity: 1, scale: 1 }}
									key={lap}
									className='glass-panel glass-quiet flex items-center justify-center rounded-xl px-6 py-4'>
...
function SevenSegmentDigit({ value, className }: SevenSegmentDigitProps) {
	...
	return (
		<motion.svg
			...
			transition={{ duration: 0.16, ease: APPLE_EASE_OUT }}>
			<path
