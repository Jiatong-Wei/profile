...
export async function loadBlog(slug: string): Promise<LoadedBlog> {
	...
	const configRes = await fetch(withSiteBase(`/blogs/${encodeURIComponent(slug)}/config.json`))
	if (configRes.ok) {
		try {
	...
	// Load index.md
	const mdRes = await fetch(withSiteBase(`/blogs/${encodeURIComponent(slug)}/index.md`))
	if (!mdRes.ok) {
