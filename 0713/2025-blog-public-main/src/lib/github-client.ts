...
import { withRepoSourceRoot } from '@/lib/repo-path'

export const GH_API = 'https://api.github.com'

function encodeApiPath(value: string): string {
...
export async function signAppJwt(appId: string, privateKeyPem: string): Promise<string> {
	const { createAppJwt } = await import('@/lib/sign-app-jwt')
	return createAppJwt(appId, privateKeyPem)
}
...
export async function getFileSha(token: string, owner: string, repo: string, path: string, branch: string): Promise<string | undefined> {
	const repoPath = withRepoSourceRoot(path)
	const res = await fetch(`${GH_API}/repos/${owner}/${repo}/contents/${encodeApiPath(repoPath)}?ref=${encodeURIComponent(branch)}`, {
		headers: {
...
export async function putFile(token: string, owner: string, repo: string, path: string, contentBase64: string, message: string, branch: string) {
	...
	const repoPath = withRepoSourceRoot(path)
	const res = await fetch(`${GH_API}/repos/${owner}/${repo}/contents/${encodeApiPath(repoPath)}`, {
		method: 'PUT',
		headers: {
			Authorization: `Bearer ${token}`,
			Accept: 'application/vnd.github+json',
...
export async function getRef(token: string, owner: string, repo: string, ref: string): Promise<{ sha: string }> {
	const res = await fetch(`${GH_API}/repos/${owner}/${repo}/git/ref/${encodeApiPath(ref)}`, {
		headers: {
			Authorization: `Bearer ${token}`,
			Accept: 'application/vnd.github+json',
			'X-GitHub-Api-Version': '2022-11-28'
...
export async function createTree(token: string, owner: string, repo: string, tree: TreeItem[], baseTree?: string): Promise<{ sha: string }> {
	const repoTree = tree.map(item => ({ ...item, path: withRepoSourceRoot(item.path) }))
	const res = await fetch(`${GH_API}/repos/${owner}/${repo}/git/trees`, {
		...
		body: JSON.stringify({ tree: repoTree, base_tree: baseTree })
	})
	if (res.status === 401) handle401Error()
	if (res.status === 422) handle422Error()
...
export async function updateRef(token: string, owner: string, repo: string, ref: string, sha: string, force = false): Promise<void> {
	const res = await fetch(`${GH_API}/repos/${owner}/${repo}/git/refs/${encodeApiPath(ref)}`, {
		method: 'PATCH',
		headers: {
			Authorization: `Bearer ${token}`,
			Accept: 'application/vnd.github+json',
...
export async function readTextFileFromRepo(token: string, owner: string, repo: string, path: string, ref: string): Promise<string | null> {
	const repoPath = withRepoSourceRoot(path)
	const res = await fetch(`${GH_API}/repos/${owner}/${repo}/contents/${encodeApiPath(repoPath)}?ref=${encodeURIComponent(ref)}`, {
		headers: {
			Authorization: `Bearer ${token}`,
			Accept: 'application/vnd.github+json',
			'X-GitHub-Api-Version': '2022-11-28'
...
export async function listRepoFilesRecursive(token: string, owner: string, repo: string, path: string, ref: string): Promise<string[]> {
	async function fetchPath(targetPath: string): Promise<string[]> {
		const res = await fetch(`${GH_API}/repos/${owner}/${repo}/contents/${encodeApiPath(targetPath)}?ref=${encodeURIComponent(ref)}`, {
			headers: {
				Authorization: `Bearer ${token}`,
				Accept: 'application/vnd.github+json',
				'X-GitHub-Api-Version': '2022-11-28'
	...
	return fetchPath(withRepoSourceRoot(path))
}
